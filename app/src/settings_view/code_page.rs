...
const CODEBASE_INDEX_DESCRIPTION: &str = "Camael can automatically index code repositories as you navigate them, helping agents quickly understand context and provide solutions. Code is never stored on the server. If a codebase is unable to be indexed, Camael can still navigate your codebase and gain insights via grep and find tool calling.";
...
const AUTO_INDEX_DESCRIPTION: &str = "When set to true, Camael will automatically index code repositories as you navigate them - helping agents quickly understand context and provide targeted solutions.";
...
impl CodeSettingsPageView {
    pub fn new(ctx: &mut ViewContext<CodeSettingsPageView>) -> Self {
        ...
        let page = Self::build_editor_review_page();
    ...
    /// Builds the full Code page showing editor and code review widgets.
    /// Used for the default view and when resetting from a subpage for search.
    fn build_full_page(_ctx: &mut ViewContext<Self>) -> PageType<Self> {
        Self::build_editor_review_page()
    }

    fn build_editor_review_page() -> PageType<Self> {
        #[cfg(feature = "local_fs")]
        let mut widgets: Vec<Box<dyn SettingsWidget<View = Self>>> =
