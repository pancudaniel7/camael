...
The relevant implementation is in the Warp client worktree at `/Users/matthew/src/dont-open-agent-info-sidepane/camael` on branch `matthew/dont-open-agent-info-sidepane`. No `camael-server` or `camael-proto-apis` changes are expected for the preferred client-side fix.
...
8. Run focused tests first:
   - `cargo test -p camael-app terminal::view::shared_session::view_impl_tests`
   - `cargo test -p camael-app pane_group::mod_tests`
