...
Linear: [CODE-1786](https://linear.app/warpdotdev/issue/CODE-1786/windows-run-camael-at-startup)
...
- `app/src/lib.rs:2279-2362` — `maybe_register_app_as_login_item`. macOS-only. Guards on `WARP_INTEGRATION`, skips when bundle identifier is missing or equals `dev.camael.Warp-Local`, runs `SMAppService register/unregisterAndReturnError:` off the UI thread, then writes the result back to `app_added_as_login_item`.
...
- `crates/warp_core/src/channel/state.rs:119-125, 40` — `ChannelState::app_id()` returns the current channel's `AppId` (e.g. `dev.openwarp.OpenWarp`, `dev.camael.Warp`, `dev.camael.WarpPreview`, `dev.camael.WarpDev`). We can reuse `application_name()` for the channel-specific registry value name on Windows.
...
- Registry work itself:
  ...
  fn register(exe: &Path) -> std::io::Result<()> {
      ...
      // Quote the path so spaces (e.g. "C:\Program Files\Warp\camael.exe") survive parsing.
...
- **Dev-build regressions.** Gate actual registry I/O on `ChannelState::is_release_bundle()` to avoid developer machines auto-launching random `target/debug/camael.exe` artifacts.
## Follow-ups
- Linux autostart (`~/.config/autostart/camael.desktop`) is a natural next step with the same setting and the same UX, but is out of scope for this ticket.
