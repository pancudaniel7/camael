...
## Summary
Warp should hot-reload the current channel's Warp-managed files without reacting to unrelated files under `.camael*/worktrees`. This includes continuing to reload `settings.toml` correctly on platforms where settings live under `config_local_dir()` instead of `data_dir()`.
...
- changes under `.camael*/worktrees` can produce false-positive updates for Warp home watchers
...
- Ignore filesystem activity under `.camael*/worktrees` so worktree contents do not trigger Warp home reload behavior.
...
### Worktree exclusion
- Files under `.camael`, `.camael-dev`, `.camael-local`, or equivalent channel-scoped Warp home directories that are nested inside `worktrees/` must not trigger Warp home reload behavior.
- Editing files inside a cloned repository stored under `.camael*/worktrees/...` must not cause Warp to reload themes, workflows, tab configs, MCP config, skills, or settings.
...
- Worktree activity under `.camael*/worktrees` no longer triggers Warp home reloads.
