...
Add install and update chip support for the OpenCode Warp plugin (`opencode-camael`), matching the existing Claude Code chip UX. Clicking the chip opens a modal with manual installation/update instructions.
...
The `opencode-camael` npm package must be published before this feature ships. The name is currently available on npm.
...
- Steps:
  1. "Add the plugin to your config" — copyable snippet:
     ```json
     { "plugin": ["opencode-camael"] }
     ```
     with explanatory text: "Add `"opencode-camael"` to the `plugin` array in your `opencode.json` (project root) or `~/.config/opencode/opencode.json` (global)."
...
- Since the opencode-camael plugin has never been released, there is no legacy version that predates version reporting. Every installed version will report `plugin_version`.
...
- Steps:
  1. "Remove the cached plugin" — copyable command: `rm -rf ~/.cache/opencode/node_modules/opencode-camael`
