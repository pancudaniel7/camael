...
- `minimum_plugin_version()` → `"0.1.0"` (keep in sync with `opencode-camael` npm package version)
- `install_instructions()` → static `PluginInstructions` with title "Install Warp Plugin for OpenCode", steps to add `"opencode-camael"` to the `plugin` array in `opencode.json` + restart
- `update_instructions()` → static `PluginInstructions` with title "Update Warp Plugin for OpenCode", steps to `rm -rf ~/.cache/opencode/node_modules/opencode-camael` + restart
...
6. User follows steps. On restart, Bun re-resolves `opencode-camael` from npm (cache was cleared), installs latest. Plugin connects with `"0.2.0"`. Chip disappears.
...
- **Publish `opencode-camael` to npm:** Must happen before this feature ships.
