...
3. Add an activation-aware lookup for bundled skill reads.
   ...
   - Use the activation-aware path in `ReadSkillExecutor` so `read_skill` cannot expose `@camael-skill:feedback` content when the setting is disabled.
...
9. Run targeted tests:
   - `cargo test -p camael skill_manager_tests`
   - `cargo test -p camael read_skill_tests`
   - `cargo test -p camael ai_page_tests`
...
- Stale skill context could still reference `@camael-skill:feedback`. Mitigation: guard direct `read_skill` execution with the same activation state used by skill listing.
...
- Agent A: local execution in `/Users/captainsafia/code/camael`, same branch, owns `skill_manager_tests` and `read_skill_tests`.
- Agent B: local execution in a separate worktree, for example `/Users/captainsafia/code/camael-app-4549-ui-tests` on branch `safia/app-4549-ui-validation`, owns `ai_page_tests` and settings UI review.
