...
- GUI process: the Warp app log for the active build flavor (e.g. `~/Library/Logs/dev.camael.Warp-Stable/camael.log` on macOS).
...
- Server-side: `stream_agent_events` requests in the dev `camael-server` access log — one request per active SSE; new requests on reconnect; disconnects when the client closes the stream.
