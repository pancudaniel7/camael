...
- Consider exposing the image-attached placeholder convention as a named sentinel (for example, `<!-- camael-feedback:image-N -->`) if we ever want to post-process the submitted issue to validate that users replaced the placeholders before submission.
