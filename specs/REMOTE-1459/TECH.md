...
- Live shared-session viewer join (from `camael://shared_session/{id}`, management view `OpenAmbientAgentSession`, or conversation list `OpenAmbientAgentSession`).
- Transcript viewer load (from `camael://conversation/{id}` or management / conversation-list `OpenConversationTranscriptViewer`).
...
- **Claude / Gemini, live, shared session link** — start a run on one client, join from another via `camael://shared_session/{id}`. Confirm the joining client lands in agent view.
...
- **Claude / Gemini, completed, conversation link** — same via `camael://conversation/{id}`.
