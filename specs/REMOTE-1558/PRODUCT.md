...
5. While handoff-compose mode is active and the prompt is empty, the input shows explanatory camael/hint text so the user understands that the prompt will start a cloud run. Once the user has typed any prompt text, this camael/hint text is hidden.
