...
1. Worktrees under paths like `~/.camael-dev/worktrees/camael-internal/...` get indexed as codebases and then silently added to the colors list, flooding the settings panel with entries the user never wanted to manage.
...
- Manual validation:
  1. With several worktrees under `~/.camael-dev/worktrees/camael-internal/...` indexed, confirm none appear in the Directory tab colors list on a fresh profile.
