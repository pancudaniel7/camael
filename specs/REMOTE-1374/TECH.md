...
Presubmit: run `./script/presubmit` (fmt, clippy `-D warnings`, test suite). The touched crates are `warp_cli` and `camael` (app); the test suite should run quickly for these.
