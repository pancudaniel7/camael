...
- **Integration test for `Initialize` round trip**: Spawn `camael remote-server` as a child process, create a `RemoteServerClient` over the child's stdin/stdout, call `initialize()`, assert `server_version` is non-empty. The test lives in `app/tests/remote_server_tests.rs` — because the `camael` binary is a `[[bin]]` target in the `app` crate, cargo automatically builds it before running these tests.
- **Shutdown test**: Send an `Initialize`, then close the client's write end. Assert the server process exits cleanly (exit code 0).
- **Build validation**: `cargo build -p camael` produces a binary with the `remote-server` subcommand. `cargo clippy` and `cargo fmt` pass.
