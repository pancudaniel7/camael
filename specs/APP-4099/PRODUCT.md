...
The fix is to use a clean redirect URI (`camael://mcp/oauth2callback`) throughout the flow and route incoming callbacks using the standard OAuth `state` parameter instead.

## Problem

When a user attempts to connect to an OAuth-protected MCP server, Warp:

1. Registers the client via DCR, sending `redirect_uris: ["camael://mcp/oauth2callback?server_id=<uuid>"]`
2. Some OAuth servers normalize this URI during registration (stripping the query parameter), storing only `camael://mcp/oauth2callback`
3. Warp then sends the authorization request with `redirect_uri=camael://mcp/oauth2callback?server_id=<uuid>`
...
4. The browser redirects to `camael://mcp/oauth2callback?code=<code>&state=<csrf_token>` — no `server_id` appended
