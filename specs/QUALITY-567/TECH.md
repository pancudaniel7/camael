...
- `pub struct OrchestrationPillBar` — implements `View` with `Entity::Event = ()`.
  - Holds `agent_view_controller: ModelHandle<AgentViewController>` and `mouse_states: HashMap<AIConversationId, MouseStateHandle>` for persistent per-pill hover state (per camael.md's `MouseStateHandle` rule — inline `MouseStateHandle::default()` would silently break clicks).
...
- **Mouse state lifetime.** Constructing `MouseStateHandle::default()` inline at render time silently zeros out hover state every frame. Per-pill state lives in the view's `mouse_states` map; the parent crumb's state is sourced from `TerminalViewMouseStates`. This pattern is enforced by the existing camael.md guidance.
