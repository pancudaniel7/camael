...
**Constants:**
- `EXTENSION_REPO: &str = "https://github.com/warpdotdev/gemini-cli-camael"` — install source.
- `EXTENSION_NAME: &str = "gemini-camael"` — the installed directory name under `~/.gemini/extensions/`. Used for `gemini extensions update gemini-camael`.
...
`is_installed()` — checks if `~/.gemini/extensions/gemini-camael/gemini-extension.json` exists and parses as valid JSON. `fs::read_to_string` follows symlinks, so `gemini extensions link` is handled.

`installed_version()` — reads `~/.gemini/extensions/gemini-camael/gemini-extension.json`, parses the `version` field. The JSON structure is flat: `{"name": "camael", "version": "1.0.0", ...}`.
...
`install()`:
```
gemini extensions install https://github.com/warpdotdev/gemini-cli-camael --consent
```
`--consent` skips the interactive security confirmation prompt.

`update()`:
```
gemini extensions update gemini-camael
```
...
Install instructions: single step — `gemini extensions install https://github.com/warpdotdev/gemini-cli-camael --consent`.

Update instructions: single step — `gemini extensions update gemini-camael`. Post-install note: "Restart Gemini CLI to activate the update."
...
3. Footer: `plugin_chip_kind()` → no listener, `is_installed()` checks `~/.gemini/extensions/gemini-camael/gemini-extension.json` → not found → `PluginChipKind::Install`.
4. `should_use_manual_mode()` → `false` (auto-install, local, no prior failure).
5. User clicks → `handle_plugin_operation()` → `gemini extensions install https://github.com/warpdotdev/gemini-cli-camael --consent`.
...
3. User clicks → `gemini extensions update gemini-camael` → success → restart → version `"1.1.0"` → chip gone.
...
- **Publish `warpdotdev/gemini-camael` to GitHub** — must happen before shipping to external users.
