...
Add install and update chip support for the Gemini CLI Warp plugin (`gemini-camael`), matching the existing Claude Code chip UX. Enable the listener so Warp processes structured notifications from Gemini sessions (session status, permission requests, task completion).

## Problem

The `gemini-camael` extension already exists and uses the same structured OSC 777 notification protocol as Claude Code (`camael://cli-agent` title, JSON body with `agent: "gemini"`). However, Warp currently ignores these notifications because:
...
- Changing the gemini-camael plugin itself (it's maintained separately in `warpdotdev/gemini-cli-camael`).
...
- **Hooks**: Defined in `hooks/hooks.json` within the extension directory (not in the manifest). The gemini-camael plugin uses `SessionStart`, `AfterAgent`, `Notification`, `BeforeAgent`, and `AfterTool` hooks.
...
- Clicking triggers auto-install via `gemini extensions install https://github.com/warpdotdev/gemini-cli-camael --consent`.
...
- Steps:
  1. "Install the Warp extension" — command: `gemini extensions install https://github.com/warpdotdev/gemini-cli-camael --consent`
...
- Clicking triggers auto-update via `gemini extensions update gemini-camael`.
  - The extension name is `gemini-camael` (the installed directory name under `~/.gemini/extensions/`).
...
- Subtitle: "Run the following command, then restart Gemini CLI."
- Steps:
  1. "Update the Warp extension" — command: `gemini extensions update gemini-camael`
...
1. **Filesystem check**: `~/.gemini/extensions/gemini-camael/gemini-extension.json` — read the `version` field to determine if the plugin is installed and whether it's outdated. Symlinks are followed (supports `gemini extensions link`).
...
5. Clicking the update chip runs `gemini extensions update gemini-camael` and shows a success/failure toast.
