...
- Manual validation (dogfood build, `NetworkLogConsole` enabled by default):
  ...
  - Disable `ContextFlag::NetworkLogConsole` (e.g. camael-home-link-only mode) → settings link hidden, keybinding unavailable. Covers invariant 3.
...
- WASM build: `cargo check --target wasm32-unknown-unknown -p camael` (or the repo's standard wasm command) to confirm the removed `local_fs` cfg split didn't leave wasm-only gaps.
