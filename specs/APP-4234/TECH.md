...
1. Cross-compiles the Oz CLI for the test VM target:
   ```
   cargo build -p camael --bin camael --target x86_64-unknown-linux-musl \
     --profile dev-remote \
     --features release_bundle,crash_reporting,standalone,agent_mode_debug
   ```
   Same build command as `script/deploy_remote_server`.

2. Uploads the binary to `ubuntu-14-04` at `~/.camael-dev/remote-server/oz-dev` via `sshpass` + `scp` through the GCP IAP tunnel (the test VM uses password auth; `sshpass` provides it non-interactively). Uses the same proxy command as the SSH integration tests (`app/src/integration_testing/subshell/util.rs:2`).
...
8. Create a git repo on the remote via `execute_command("mkdir -p /tmp/camael-test-repo && cd /tmp/camael-test-repo && git init -b main ...")`
9. `execute_command("cd /tmp/camael-test-repo")` — triggers CWD change → `navigate_to_directory`
...
After Test A setup, plus:
8. `write_file_via_remote_server(0, "/tmp/camael-rs-test-file.txt", "hello from proto")` — writes via proto
9. `execute_command("cat /tmp/camael-rs-test-file.txt")` — reads back via `RunCommand`, asserts `"hello from proto"`
...
9. `cd /tmp/camael-lazy-repo` → triggers `NavigatedToDirectory` and full indexing
