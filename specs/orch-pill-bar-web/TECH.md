...
- `camael-server/router/handlers/public_api/agent_webhooks.go (1807-1814)` — `ancestor_run_id` filter validation
- `camael-server/public_api/types/types.gen.go (1579-1658)` — `RunItem` struct with `SessionId`, `SessionLink`, `State`, `Title`, `AgentConfig`, `Artifacts`
- `camael-server/authz/engine.go (556-605)` — `aiTaskPolicies`, ownership-based `ViewAction`
...
The viewer queries existing camael-server REST APIs for child metadata and status. Each discovered child gets its own hidden shared-session viewer pane (its own `TerminalView`, `TerminalManager`, and child-session `Network`), reusing the pattern that local orchestration uses for remote child agents. Pill clicks navigate via the existing `SwapPaneToConversation` mechanism. No changes to `session-sharing-protocol` or `camael-proto-apis`.
...
**Pros:**
- Repos touched: **1-2** (camael client + possibly minor camael-server authz). No `session-sharing-protocol` or `camael-proto-apis` changes.
...
1. Extend `StreamInit` in `camael-proto-apis/apis/multi_agent/v1/response.proto` with optional orchestration fields: `agent_name`, `agent_role`, `parent_agent_id`, `harness_type`, `working_directory`, `agent_description`.
...
**Cons:**
- Repos touched: **4** (session-sharing-protocol, camael-proto-apis, camael-server, camael client).
...
### Layer 1: camael-server — authz (no changes needed)
...
### Layer 2: camael client — children poller + per-child hidden shared-session panes
...
### Layer 3: camael client — make pill bar WASM-compatible
...
### Layer 4: camael client — enable pill bar for shared sessions
...
```mermaid
sequenceDiagram
    participant Server as camael-server REST API
...
With Option A, this work is entirely in the camael client repo (layers 2-4). No server changes are required (see Layer 1).
...
Branch `matthew/orch-web-ui` is already set up across `camael-server` and `camael`.
