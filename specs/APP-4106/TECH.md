...
- **Focus-follow on cd**: simulate cd-ing into `~/code/camael-server` with
  `~/code` as the ancestor. Assert `selected_item` lands on
  `~/code/camael-server`, and `pending_focus_target.scrolled` is `true`.
...
- **Click preserves file selection**: seed `~/code`, expand
  `~/code/camael-server`, select `main.rs`. Simulate a
  `DirectoriesChanged` emitting `[camael-server, code]`. Assert selection
