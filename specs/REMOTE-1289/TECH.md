...
The server is adding `POST /harness-support/notify-user` and `POST /harness-support/finish-task` endpoints (see `camael-server` spec). Third-party harnesses invoke server APIs through the `oz` CLI, not directly. We need:
...
- `../claude-code-camael-internal/plugins/oz-harness-support/` — existing plugin (skills, hooks)
...
### 5. Claude Code plugin (`../claude-code-camael-internal/plugins/oz-harness-support/`)
...
sequenceDiagram
    ...
    participant WS as camael-server
