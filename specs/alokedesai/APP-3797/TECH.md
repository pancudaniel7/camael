...
The current SSH wrapper flow creates a `RemoteCommandExecutor` that runs every generator/completion command by opening a new SSH channel through a ControlMaster socket. This is unreliable and stateless. We want to replace it with a persistent remote server binary (`~/.camael/remote-server/oz`) on the remote machine that communicates over stdin/stdout using length-prefixed protobuf messages. The binary is the Oz CLI, installed from the `/download/cli` endpoint if not already present.
...
1. **Check**: `ssh -o ControlPath={socket} placeholder@placeholder 'test -x ~/.camael/remote-server/oz && ~/.camael/remote-server/oz --version'`
2. **Install** (if check fails): pipe the following script into `bash -s` over the control socket SSH connection (mirroring how `RemoteCommandExecutor` runs commands today):
   ...
   mkdir -p "$HOME/.camael/remote-server"
   curl -fSL "$WARP_GET_URL?package=$pkg" -o "$HOME/.camael/remote-server/oz.tar.gz"
   tar -xzf "$HOME/.camael/remote-server/oz.tar.gz" -C "$HOME/.camael/remote-server"
   chmod +x "$HOME/.camael/remote-server/oz"
   ```
...
3. **Launch**: `ssh -o ControlPath={socket} placeholder@placeholder '~/.camael/remote-server/oz'` — keep the SSH channel open, forwarding stdin/stdout.
