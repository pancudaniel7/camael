...
Tab configs are TOML files in `~/.camael/tab_configs/`. They previously used a recursive `TabConfigLayout` struct with nested `panes: Vec<TabConfigLayout>` for child splits, which became impractical at depth 3+.
...
1. User creates/edits a `.toml` file in `~/.camael/tab_configs/`.
