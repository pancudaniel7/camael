...
APP-4068 partitions daemon sockets by user identity at the path level: `~/.camael[-channel]/remote-server/{identity_key}/server.sock`. The `{identity_key}` is:
...
1. **Client computes the path.** Warp (running on the local machine) already has the user's identity in memory. It builds the socket path: `~/.camael/remote-server/abc123/server.sock`. For anonymous users, `abc123` is replaced with the per-install `ExperimentId` UUID loaded from preferences.
2. **Client launches the proxy with that path.** When the user opens a remote session, the client invokes something like `oz remote-server-proxy --socket-path ~/.camael/remote-server/abc123/server.sock` on the remote host over SSH. The proxy receives the path as an opaque argv string — it does not parse, validate, or interpret the UUID segment; it just knows where the socket lives.
...
4. In-flight requests using the previous token continue — camael-server accepts a refresh overlap window of both old and new tokens.
