...
On macOS, `macos_config_dir_name()` returns `.camael` for both Stable and Preview channels, causing them to share a single config directory. This needs to change so Preview uses `.camael-preview`, with a one-time symlink migration for existing users.

## Relevant Code

- `crates/warp_core/src/paths.rs:42-51` — `macos_config_dir_name()` maps channels to directory names. The `Channel::Preview` arm currently returns `WARP_CONFIG_DIR` (`.camael`).
...
Both `data_dir()` and `config_local_dir()` use this on macOS, so all config paths (`settings.toml`, `keybindings.yaml`, `themes/`, `workflows/`, `tab_configs/`, etc.) resolve under `~/.camael` for both Stable and Preview.

The SQLite database is **not** affected — it is stored under `secure_state_dir()` (App Group container) or `state_dir()` (`~/Library/Application Support/dev.camael.Warp-Preview/`), both of which already use the bundle ID and are channel-specific.

The `WARP_CONFIG_DIR` constant (`.camael`) is also used for per-repository project configs (e.g., `./.camael/workflows`). This is unrelated to the home-directory config and must not change.

## Proposed Changes

### 1. Update `macos_config_dir_name()` — `crates/warp_core/src/paths.rs`

Change the Preview arm to return `.camael-preview`:
...
Add a new module in the `camael` (app) crate that owns the migration. The migration is app startup logic, not a general path utility, so it lives in the app crate alongside other one-time migrations (e.g., `app/src/persistence/sqlite.rs`). The `warp_core::paths::data_dir()` helper is already channel-aware, so the module does not need access to the private `macos_config_dir_name()` in `warp_core`.
...
The directory's existence is the migration marker — no separate marker file is needed. Subsequent launches see `~/.camael-preview` already exists and no-op.
...
- Before `ensure_warp_watch_roots_exist()` would `create_dir_all` on `~/.camael-preview` (which would make the `new_dir.exists()` check true and skip migration).
...
sequenceDiagram
    ...
    Migrate->>FS: Check ~/.camael-preview exists?
    alt Already exists
        Migrate-->>Init: No-op
    else Does not exist
        Migrate->>FS: Check ~/.camael exists?
        alt Does not exist
            Migrate-->>Init: No-op (fresh install)
        else Exists
            Migrate->>FS: create_dir(~/.camael-preview)
            Migrate->>FS: read_dir(~/.camael)
            loop Each top-level entry
                Migrate->>FS: symlink(~/.camael-preview/X -> ~/.camael/X)
            end
        end
    end
    Init->>Ensure: ensure_warp_watch_roots_exist()
    Ensure->>FS: create_dir_all(~/.camael-preview) [no-op if exists]
    Init->>Init: Continue startup (WarpConfig, settings, etc.)
```

## Risks and Mitigations

### Symlink breakage if `~/.camael` is deleted
If a user deletes `~/.camael` after migration, all symlinks in `~/.camael-preview` become dangling. Warp's existing directory-creation logic (`ensure_warp_watch_roots_exist`) will recreate `~/.camael-preview` subdirectories as needed, and missing files will be treated as defaults. This is the same behavior as a fresh install.

**Mitigation**: Acceptable degradation. No action needed.

### Filesystem watcher and symlinks
The `notify` crate (used by `BulkFilesystemWatcher`) watches `~/.camael-preview` recursively. For symlinked directories, `notify` with `RecursiveMode::Recursive` follows symlinks by default on macOS (using `kqueue`/`FSEvents`). Changes to the underlying files in `~/.camael` will trigger events on the watched path in `~/.camael-preview`.

**Mitigation**: Verify in manual testing. If events are missed, we may need to also watch `~/.camael` from Preview, but this is unlikely to be needed.
...
If Stable creates a new entry in `~/.camael` after Preview's `read_dir` call but before symlinking completes, that entry won't have a symlink. This is a negligible window and the entry would only be visible in Stable.
...
1. **Migration creates symlinks**: Set up a temp dir with mock `.camael` contents, run migration, assert symlinks exist and point to correct targets.
2. **Migration skips when target exists**: Pre-create the target directory, run migration, assert no symlinks are created.
3. **Migration skips when source missing**: Run migration without a `.camael` directory, assert no errors.
...
1. In `with_setup`, populate the hermetic home's `.camael/` directory with representative entries (e.g., `keybindings.yaml`, `themes/`, `workflows/`, `settings.toml`, a `.DS_Store`).
2. In `with_setup`, call `camael::integration_testing::preview_config_migration::run_config_dir_symlink_migration` (since the integration channel is `Integration`, not `Preview`, the public entry point would no-op — so we use the integration-testing hook).
3. In a `TestStep`, assert:
   - `~/.camael-preview` exists and is a directory (not a symlink).
   - Each expected entry in `~/.camael-preview` is a symlink pointing to the corresponding entry in `~/.camael`.
...
### Manual validation
- Build a Preview binary, launch with existing `~/.camael`, verify:
  - `~/.camael-preview` is created with symlinks.
  - Settings, keybindings, themes all load correctly.
  - Creating a new tab config in Preview appears in the expected location.
- Launch Stable after, verify `~/.camael` is unmodified.
