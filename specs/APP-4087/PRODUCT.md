...
## Problem
Before APP-3945, Warp skill discovery could find skills in Warp’s home config directory, such as `~/.camael/skills`. After APP-3945, `SkillProvider::Warp` stopped going through generic home-provider watching and instead relied on the centralized Warp watcher and app data paths. That prevented broad recursive watches under `~/.camael`, which was intended, but it also meant that home-relative Warp skill directories stopped being considered when app data was somewhere else.
Linux and Windows expose the bug because their app data directories differ from Warp’s home-relative `.camael*` config directories. A Stable user can therefore have a valid skill at `~/.camael/skills/foo/SKILL.md`, run `oz agent run --skill foo`, and see the skill fail to resolve because the resolver and watcher are looking in the platform app data directory.
macOS hides most of this because Stable and Preview commonly resolve app data to `~/.camael`, while Dev and Local resolve to environment-specific home directories such as `~/.camael-dev` and `~/.camael-local`. The desired behavior is not simply “always use `~/.camael`”; it is “use the home-relative Warp config directory for the current Warp app environment.”
While investigating Skills, we checked MCP because its global config has the same shape. Warp MCP config should use the same environment-aware home config directory as Skills, e.g. `~/.camael/.mcp.json`, `~/.camael-dev/.mcp.json`, or `~/.camael-local/.mcp.json` depending on channel/profile.
## Goals
- Preserve the APP-3945 invariant that Warp does not recursively watch `.camael*/worktrees`.
...
- Migrating existing files between platform app data directories and home-relative `.camael*` directories.
...
## User Experience
### Warp home skills
- A Stable user can store a skill at `~/.camael/skills/<skill-name>/SKILL.md`.
- Dev, Local, Integration, OpenWarp, and profiled builds use their own home-relative Warp config directories, such as `~/.camael-dev/skills`, `~/.camael-local/skills`, or `~/.camael-local-<profile>/skills`.
...
- A user can configure file-based MCP servers for the current Warp app environment at `<camael-home-config-dir>/.mcp.json`.
- Examples include `~/.camael/.mcp.json`, `~/.camael-dev/.mcp.json`, and `~/.camael-local/.mcp.json`.
...
### Worktree exclusion
- Activity under `.camael*/worktrees` must not trigger reloads for themes, workflows, tab configs, settings, skills, or MCP config.
- Supporting Warp home skills must not be implemented by recursively watching all possible `.camael*` directories.
- Supporting Warp home MCP config must not be implemented by recursively watching all possible `.camael*` directories.
### Existing app paths
...
## Success Criteria
- `oz agent run --skill <name>` can resolve `<camael-home-config-dir>/skills/<name>/SKILL.md` from a cold start.
...
- File-based MCP detection includes `<camael-home-config-dir>/.mcp.json` as a user-scoped Warp provider config when present.
- Dev/Local/Profiled builds use isolated `.camael*` home config directories instead of Stable’s `~/.camael` directory.
- No code path treats non-macOS XDG/AppData `data_dir()/skills` or `data_dir()/.mcp.json` as Warp home Skills or MCP sources.
- No code path reintroduces a generic recursive watcher rooted at `~/.camael`.
- `.camael*/worktrees` changes remain excluded from Warp-managed reload behavior.
## Validation
...
- Add or update unit coverage for MCP path classification so `<camael-home-config-dir>/.mcp.json` is recognized as a Warp MCP config path.
...
- Use only hardcoded `~/.camael` for Skills/MCP. Rejected because it loses Dev/Local/Profile environment isolation.
- Keep using `data_dir()` for Skills/MCP. Rejected because non-macOS app data paths are XDG/AppData paths, not Warp’s home-relative `.camael*` config paths.
- Use `config_local_dir()` for Skills/MCP. Rejected because non-macOS config-local paths are also platform project directories, not home-relative `.camael*` paths.
...
- Re-add Warp to generic home-provider watchers. Rejected because that watcher shape can recursively watch `.camael*` parents and reintroduce `.camael*/worktrees` churn.
- Watch all of `~/.camael` recursively and filter in consumers. Rejected because it recreates the broad watcher shape APP-3945 was designed to remove.
## Open Questions
