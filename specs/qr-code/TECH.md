...
Default filename suggestion: `camael-session-qr-code-<session-id>.png` for session targets. If the session id is unavailable in a future target shape, fall back to `camael-session-qr-code.png`.
