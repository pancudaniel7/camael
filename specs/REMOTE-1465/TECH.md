...
- `camael-agent-docker/snapshot-declarations.sh` — the existing script's dedup step reads only `repo` JSONL lines into its seen-set, so `file` lines written by the driver are left intact across repeated script invocations.
...
### 4. No changes to `snapshot-declarations.sh` or `camael-agent-docker`

The existing script's dedup step only tracks `repo` lines (`camael-agent-docker/snapshot-declarations.sh:42`), so driver-written `file` lines survive re-invocation. Nothing in the Docker image or `entrypoint.sh` needs to change.
