...
31. The v1 product assumes the remote daemon can reach `app.camael.dev`; that assumption has been checked with the initial target enterprise environments.

32. If the remote daemon cannot reach `app.camael.dev`, remote indexing fails with a user-readable error such as "Warp could not reach the backend from this remote host." The user can retry after fixing network access.
