...
- **Project rules** — `camael.md` / `AGENTS.md` walked up from the working directory. Indexed by `ProjectContextModel`, sent on every query inside `AIAgentContext::ProjectRules`.
...
- Global rule + project `camael.md` for the same path → both appear in `active_rules`, ordered global first. Covers invariants 8, 9.
- Global rule + project `camael.md` and `AGENTS.md` in the same dir → project `camael.md` shadows project `AGENTS.md`; global is appended. Covers invariant 9.
