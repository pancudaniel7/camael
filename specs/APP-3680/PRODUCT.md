...
A new modal ("Create your default tab config") appears after onboarding completes, overlayed on the terminal. It collects three inputs — session type, directory, and worktree preference — then creates a persistent tab config TOML in `~/.camael/tab_configs/` and opens it in the current tab.
...
- Always persist the configuration as a reusable tab config TOML in `~/.camael/tab_configs/`.
...
1. Writes a new TOML file to `~/.camael/tab_configs/`. The file is named `startup_config.toml` (or `startup_config_1.toml`, `startup_config_2.toml`, etc. if the name is taken).
...
2. Selecting "Terminal" + a directory + "Get warping" writes a tab config TOML to `~/.camael/tab_configs/` and replaces the current tab with a session in that directory.
