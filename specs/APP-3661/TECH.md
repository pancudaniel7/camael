...
Must be kept in sync with the plugin version in `warpdotdev/claude-code-camael`. Add a comment on the constant pointing to the plugin repo, and a reciprocal comment in the plugin repo's README.
...
- Steps:
  1. `claude plugin marketplace remove claude-code-camael`
  2. `claude plugin marketplace add warpdotdev/claude-code-camael`
  3. `claude plugin install camael@claude-code-camael`
