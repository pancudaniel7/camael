...
Issue: https://github.com/warpdotdev/camael-external/issues/1063
...
2. `app/src/settings_view/ai_page.rs`
   ...
   - In `GlobalAIWidget::search_terms`, keep existing terms (including `"oz"`
     for legacy muscle memory, allowed by Behavior #7) and keep `"camael agent"`
...
- `cargo fmt` and `cargo clippy --workspace --all-targets --all-features --tests
  -- -D warnings` must pass (per `camael.md` PR workflow).
...
- Behavior #7: search within the settings modal using each of
  `"camael agent"`, `"ai"`, `"agent"`, `"oz"` (should still reach the subpage) and
...
- Risk: stale comments inside `agent_slide.rs` that still reference "Disable Oz"
  ...
  comments describing them are acceptable to leave as-is per `camael.md`.
