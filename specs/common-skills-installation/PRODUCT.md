...
14. Project-local installs and global installs are mutually exclusive for common skills. If common skills are detected in both the Warp checkout and the user's global skill directory, install and verification flows fail with an actionable error telling the developer to remove one copy before continuing. The error should suggest using `remove_common_skills --repo-root <camael-checkout>` to remove the project-local copy or `remove_common_skills --repo-root <camael-checkout> --global` to remove the global copy.
