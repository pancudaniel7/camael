...
- **Cloud→cloud handoff and rehydration** (REMOTE-1290): `snapshots/{run_id}/{execution_id}/` GCS layout, the `<system-message>`-wrapped `UserQuery` rehydration prompt injected by `logic/ai/multi_agent/runtime/interceptors/input.go:433` via `ResolveHandoffRehydrationPrompt` in `../camael-server/logic/ai/ambient_agents/handoff_rehydration.go`. Server discovers snapshot files by GCS path convention (`ListSnapshotFiles` in `../camael-server/logic/ai/ambient_agents/attachment_storage.go:281`), no DB column needed.
...
sequenceDiagram
    ...
    participant API as camael-server (public API)
...
```
Location: alongside `UpsertAIConversationMetadata` / `CreateThirdPartyAIConversation` in `../camael-server/logic/ai_conversation_object.go`. Steps:
...
Extend `RunAgentRequest` in `../camael-server/router/handlers/public_api/agent_webhooks.go:199` with two new fields:
```go path=null start=null
...
- **Fork on a very large conversation.** `ForkConversation` copies the source conversation object inside GCS. *Mitigation:* use the server-side `CopyConversationDataInGCS` path so bytes do not round-trip through the camael-server process.
