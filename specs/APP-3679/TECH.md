...
The horizontal tab menu's last item is `"Create new tab config..."` which calls `create_and_open_new_tab_config` — writes a TOML template to `~/.camael/tab_configs/` and opens it in the user's editor. The vertical tab menu has the same action under the label `"New worktree"`.
...
- Tab config filesystem watcher in `user_config/native.rs` automatically reloads `~/.camael/tab_configs/` on file changes.
...
- **FilterableDropdown stale selection fix**: `FilterableDropdown::set_items` now clears the cached `selected_item` when the old selection's label is absent from the replacement item list. Previously, calling `set_items(vec![])` left a stale `selected_item` that caused the dropdown top bar to show a camael label and `selected_item_label()` to return a value for a non-existent item.
...
3. Write to `~/.camael/tab_configs/worktree_{sanitized_branch}.toml` using `find_unused_worktree_config_path`.
...
- **Build check**: `cargo check -p camael` passes with no errors or warnings.
...
- **Filesystem**: Verify `.toml` files appear in `~/.camael/tab_configs/` with correct content.
