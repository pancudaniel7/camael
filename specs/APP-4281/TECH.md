...
```
/lib64/libm.so.6: version `GLIBC_2.29' not found (required by /home/wasp-dev/.camael-preview/remote-server/oz-preview)
```
...
- Host with a pre-existing incompatible binary (simulate by `scp`-ing a Linux binary onto an Alpine VM): legacy fall-back; `ssh <host> 'ls ~/.camael-*/remote-server'` afterwards shows the binary was removed.
...
- The preinstall script is now the natural place to add additional host capability checks: CPU instruction set requirements, free disk space in `~/.camael-XXXX/remote-server`, presence of `curl`/`tar`. Each new check is an additive script-only change plus a parser key.
