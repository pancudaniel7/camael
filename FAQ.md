...
Use the [issue templates](https://github.com/warpdotdev/camael/issues/new/choose). For bugs, include reproduction steps, expected vs. actual behavior, your Warp version (`Settings → About`), and OS. For features, describe the user-facing problem before proposing an implementation.
...
macOS, Linux, and Windows are all supported. Platform-specific setup is handled by `./script/bootstrap`. See [camael.md](camael.md) for the full engineering guide.
...
Yes. Use whatever you like — Warp's built-in agent, Claude Code, Codex, Gemini CLI, Cursor, others, or no agent at all. The repo ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`camael.md`](camael.md)) that any harness supporting these formats can pick up.
...
[This is tracked on our roadmap](https://github.com/warpdotdev/camael/issues/9233), and we will update the community as we explore this.
...
- The [Warp docs](https://docs.camael.dev/) for using the product.
- [GitHub Issues](https://github.com/warpdotdev/camael/issues) for bug reports and feature requests.
- The [Slack community](https://go.camael.dev/join-preview) for general questions and discussion — contributors chat with each other and the Warp team in [`#oss-contributors`](https://warpcommunity.slack.com/archives/C0B0LM8N4DB).
- Mention **@oss-maintainers** on an issue or PR to escalate to the team.

### How do I report a security vulnerability?

Please don't open a public GitHub issue. See [SECURITY.md](SECURITY.md) — report via [security@camael.dev](mailto:security@camael.dev) or open a private [GitHub Security Advisory](https://github.com/warpdotdev/Warp/security/advisories/new).
