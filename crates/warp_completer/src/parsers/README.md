...
Let's say we are interested in parsing the input `camael --disable-telemetry`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "camael --disable_telemetry";
let start_offset = 0;
...
```rust
let input = "camael --disable-telemetry";

let word1 = Span::new(0,4);
let word2 = Span::new(4,5);
let word3 = Span::new(5,24);

assert_eq!(word1.slice(input), "camael");
...
let example = Spanned { item: String::from("camael"), span: Span::new(0,4) };
assert_eq!(example.item, "camael".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = String::from("camael").spanned(Span::new(0,4));
assert_eq!(example.item, "camael".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = "camael -p --disable-telemetry";
...
let expected_word1 = String::from("camael").spanned(Span::new(0,4));
...
For more involved inputs (say commands separated by `|` and/or having `;`) the lite parser will effectively create the necessary `LitePipeline`s to express it, let's explore what happens if we lite parse the input `camael config-set --extension-path="/path/to/dir" ; echo $WARP_VAR"` (*two pipelines here due to the ; character*), like so:

```rust
let input = "camael config-set --extension-path=\"/path/to/dir\" ; echo $WARP_VAR";
let start_offset = 0;
