...
Repos with Dependabot enabled: `camael-internal`, `camael-server`, `camael-terraform`, `session-sharing-server`.
...
- **Production** (`astral-field-294621`): `camael-server`, `camael-server-jobs`, `camael-server-migrations`, `session-sharing-server`, `pgbouncer-rtc`
- **Staging** (`camael-server-staging`): same repos plus `cloud-run-source-deploy`
...
#   - Title (usually contains CVE ID, e.g., "camael-server-GHSA-8r9q-7v3j-jr4g")
