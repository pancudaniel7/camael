...
description: >
  ...
  Triggers a cloud agent that runs the test-camael-ui skill.
user-invocable: true
---
...
Verify the remote URL contains `warpdotdev/camael`. If it does not, warn the user that this skill only supports the camael repository and stop.

The environment ID for the camael Dev Environment is `SVhg783GBFQHk1OfdPfFU9`.
...
- `base_prompt`: include an instruction to read and follow the `test-camael-ui` skill, followed by the verification task (see the next section)
...
The `test-camael-ui` skill is bundled, so the cloud agent has it automatically. Tell the agent to invoke it by name in the `base_prompt` (e.g. "Read and follow the test-camael-ui skill.").

### 4. Write an Effective Prompt

The prompt should tell the cloud agent:
- Which element, flow, or behavior to test
- What hardcoding or mocking is needed (see below and the test-camael-ui skill for details on sandbox constraints)
...
```
I added a migration that symlinks config from ~/.camael into ~/.camael-preview on first launch.
The migration is gated on Channel::Preview. Before building, hardcode the migration to run
regardless of channel by removing the channel check. Also create a fake ~/.camael directory
with test files. After launching Warp, verify the symlinks were created in ~/.camael-preview.
```
