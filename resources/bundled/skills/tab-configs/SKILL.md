...
Tab configs live under `~/.camael/tab_configs/` for standard Warp builds. Non-stable builds (e.g. Preview or pre-release channels) use a channel-specific variant such as `~/.camael-<channel>/tab_configs/`. Do NOT use `~/Library/Application Support/` — that is the wrong location.

To find the active data directory, inspect which `~/.camael*` directories exist on the user's machine. Each `.toml` file defines a single tab layout that appears in the `+` menu. When the user selects one, Warp opens a new tab with the configured pane layout and runs any specified commands.
...
### With parameters
Warp-generated worktree configs default to `~/.camael/worktrees/<repo-name>/<worktree-name>`. When writing a reusable config by hand, derive the repo-name segment at shell runtime with `$(basename {{repo}})`.
...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{branch_name}} $HOME/.camael/worktrees/$(basename {{repo}})/{{branch_name}} {{base_branch}}",
  "cd $HOME/.camael/worktrees/$(basename {{repo}})/{{branch_name}}",
]
