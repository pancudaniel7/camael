...
Turn rough Warp app feedback into a crisp filed issue or duplicate-issue response for `warpdotdev/camael`.

Treat Warp client, Warp app, Warp terminal, and Warp UX feedback as `warpdotdev/camael` unless the user clearly asks for a different destination.

## Overview
- Use the `gh` CLI to search for and fetch code from `warpdotdev/camael` when product or implementation context would improve the report.
...
- The helper script applies the `in-app-feedback` label to filed issues in `warpdotdev/camael` for normal Warp feedback, and requires that target repo to be passed explicitly.
...
- **Prefer `gh` CLI for code lookups.** Use `gh` to search and fetch code from `warpdotdev/camael` rather than reading the local checkout when both are available.
...
- When you decline, respond in one or two sentences that (a) say you won't file an issue, (b) name the reason in plain language, and (c) point the user at the right channel: account/billing/support concerns go to the in-app Help menu or `support@camael.dev`, community discussion goes to the Warp Slack community, and security reports go to `security@camael.dev`. Do not apologize performatively and do not offer to retry the same flow.
...
- Search the `warpdotdev/camael` repo via the `gh` CLI for matching product language, expected workflows, setting names, or UX intent when that context would make the draft more actionable.
- Search the `warpdotdev/camael` repo via the `gh` CLI for matching components, settings surfaces, feature flags, and likely code paths when implementation context would help triage.
...
- Before invoking `scripts/file_feedback_issue.py`, search issues in `warpdotdev/camael` for likely title matches using the drafted title as the primary query.
...
- The confirmation question must include the exact issue title and issue description/body that will be filed before asking for confirmation. Present them as a prerequisite to the question, using a concise format like:
  ...
  - `Would you like me to file this issue publicly in warpdotdev/camael?`
...
Use the bundled helper script `scripts/file_feedback_issue.py` to file the issue in `warpdotdev/camael` instead of calling `gh` directly. The script requires a `--use` flag that selects the filing method explicitly:
