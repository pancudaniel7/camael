...
This is the exact path Warp reads at launch — it is platform- and channel-specific (e.g. under `~/.camael*/` on macOS, under XDG config dirs like `~/.config/camael-terminal/` on Linux, and under `%LocalAppData%` on Windows). Use this path verbatim — do not infer a different one from the user's home directory layout. Create the file (and any missing parent directories) if it does not exist.
