...
Config file paths:
- **Global (user-scoped):** `~/.camael/.mcp.json`
- **Project-scoped:** `{repo_root}/.camael/.mcp.json`
...
- Project-scoped configs (`{repo_root}/.camael/.mcp.json`) run from the repo root.
- Global configs (`~/.camael/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
